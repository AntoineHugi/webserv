...
void test_42_virtual_hosting(const TestConfig& config, TestStats& stats)
{
	(void)config;
	print_test("42 EVAL: Virtual hosting with different server_name");
	...
	// Request with specific Host header
	std::map<std::string, std::string> headers;
	headers["Host"] = "server_one:8080";
	...
	{
		stats.add_pass();
		print_pass("Virtual hosting works");
	}
	else
	{
		stats.add_fail();
		print_fail("Virtual hosting failed");
	}
...
void test_42_redirection(const TestConfig& config, TestStats& stats)
{
	(void)config;
	print_test("42 EVAL: URL redirection (301/302)");
	...
	{
		stats.add_pass();
		print_pass("Redirection works with Location header");
	}
	else
	{
		stats.add_fail();
		print_fail("Redirection not working");
	}
}
...
void run_42_evaluation_tests(const TestConfig& config, TestStats& stats)
{
	...
	test_42_virtual_hosting(config, stats);
	...
	// Redirection
	test_42_redirection(config, stats);
}
