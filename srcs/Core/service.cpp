...
int Service::service_reading(std::vector<struct pollfd> &poll_fds, int i)
{
	...
	if (this->clients[poll_fds[i].fd].handle_read())
	{
		...
		if (status >= 400)  // Actual error - send error response
		{
			...
			this->clients[poll_fds[i].fd]._response.set_response_data(this->clients[poll_fds[i].fd]._response.format_response(status, 
				false, 
				clients[poll_fds[i].fd]._request._header_kv["version"]
			));
	...
	while(g_shutdown == 0)
	{
		...
		for (int i = poll_fds.size() - 1; i >= 0; i--)
		{
			...
			else
			{
				...
				if (poll_fds[i].revents & POLLIN || client.leftover_chunk() == true)
				{
					std::cout << "\n###################################################"<< std::endl;
					std::cout << "################## READING #######################"<< std::endl;
					std::cout << "###################################################\n"<< std::endl;
					...
					else
					{
						...
						if (client.can_i_process_request() == true)
								poll_fds[i].events = POLLOUT;
					}
				}
